...
DrawSystem.prototype.update = function(buffer) {
    ...
    if (this.type == DrawSystem.NBODY) {
        for (var i = 0; i < buffer.length; i+= this.ncoords) {
            if (buffer[i+0] < z*v[0] || buffer[i+0] > z*v[1] || buffer[i+1] < z*v[0] || buffer[i+1] > z*v[1])
                continue;
            var x = Math.floor(buffer[i+0]*A+c[0]);
            var y = Math.floor(buffer[i+1]*A+c[1]);
            ctx.fillStyle = this.colors[buffer[i+3]];
            ctx.beginPath();
