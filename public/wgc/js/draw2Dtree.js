...
var TreeDemo2D = (function(canv, N) {
    ...
    function redraw() {
        ...
        for (i = 0; i < p.length; i++) {
            ctx.beginPath();
            ctx.arc(p[i][0], p[i][1], 2, 0, 2*Math.PI);
            ctx.fill();
        }
        ...
        for (i = 0; i < flat.length; i++) {
            if (flat[i].min[2]+flat[i].width < 0 || flat[i].min[2] > 0)
                continue;
            ctx.fillRect(flat[i].min[0], flat[i].min[1],
                          flat[i].width, flat[i].width);
            ctx.strokeRect(flat[i].min[0], flat[i].min[1],
                          flat[i].width, flat[i].width);
        }
